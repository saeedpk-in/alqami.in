  ...
  try {
    ...
    if (existingUser) {
      if( existingUser.emailVerified) {
        return {
          success: false,
          error: "In this email there is an account. Please move to login",
        };
      } else{
        // If the user exists but is not verified, we can resend the verification email
        const emailVerificationToken = generateCode();
        const emailVerificationExpires = new Date(Date.now() + 3600000); // 1 hour

        // Update the existing user with new verification token and expiration
...
export async function sendEmailVerification(email: string, token: any) {
  const message = {
    from: `"ALQAMI OTP Verification" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: "Your ALQAMI Verification Code",
    html: `
      <div style="font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 30px;">
        <div style="max-width: 600px; margin: auto; background-color: #ffffff; padding: 40px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
          ...
          <p style="color: #999; font-size: 12px; margin-top: 40px; text-align: center;">
            This email was sent by alqami. If you have any questions, please contact our support team.
          </p>
        </div>
      </div>
    `,
