...
// import { CartItem } from "@/types&const";


// Action to handle adding or updating items in the cart
export async function addItemToCart({ cartId , userId , productId , quantity  }:{cartId?: string, userId?: string , productId: string, quantity: number; }) {
  try {
...
export async function removeItemFromCart({ cartId , userId , productId  }:{cartId?: string, userId?: string | null, productId: string; }) {
  try {
    await connectToDatabase()
    const cart = await Cart.findOne(userId ? { userId } : { cartId });
    if (!cart) {
